...
에이전트 트레이 런처로 연결/끊기, 서버 주소 변경, 업데이트, 연결 상태 확인을 할 수 있다. **[에이전트 종료]**를 누르면 서비스(프로세스)까지 완전히 멈추고, 시작 메뉴의 "PC Manager 에이전트"로 런처를 다시 열어 **[에이전트 시작]**으로 되살릴 수 있다(관리자 승격 한 번). 창의 X는 트레이로 숨길 뿐 에이전트는 계속 실행된다. 관리자 권한은 최초 설치와 재시작 때만 필요하다.
