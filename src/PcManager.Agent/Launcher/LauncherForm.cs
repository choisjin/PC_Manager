...
/// <summary>컴팩트 런처 창: 연결 상태, 서버 주소, 연결/끊기, 업데이트</summary>
internal sealed class LauncherForm : Form
{
    ...
    public LauncherForm()
    {
        ...
        var updateRow = new FlowLayoutPanel { AutoSize = true, WrapContents = false, Margin = new Padding(0, 8, 0, 0) };
        updateRow.Controls.AddRange([_update, _dashboard]);
        ...
        layout.Controls.Add(new Label { Text = "서버 주소", AutoSize = true, Margin = new Padding(0, 14, 0, 4) });
        layout.Controls.Add(_serverUrl);
        layout.Controls.Add(buttons);
        layout.Controls.Add(new Label { AutoSize = false, Height = 1, Dock = DockStyle.Fill, BackColor = Color.Gainsboro, Margin = new Padding(0, 12, 0, 8) });
        layout.Controls.Add(_info);
        ...
        {
            _error.Text = "";
            ConnectRequested?.Invoke(_serverUrl.Text);
        };
    ...
    public void ApplyStatus(LocalStatus? status, string? error)
    {
        ...
        var (color, title, detail) = status?.Status switch
        {
            ...
            _ => (TrayIcons.ErrorColor, "에이전트 서비스 꺼짐", error ?? LocalControlClient.ServiceUnavailableMessage),
        };
        _statusDot.Text = "●";
    ...
    {
        var available = _status is not null && !_status.Updating && _busyCommand is null;
    ...
    {
        // 닫기 버튼은 트레이로 숨기기 (에이전트 상태를 계속 표시)
